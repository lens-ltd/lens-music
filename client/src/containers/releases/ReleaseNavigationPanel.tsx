...
const ReleaseNavigationPanel = ({
  ...
  const navigationSteps = useMemo(() => {
    const activeTabName = activeReleaseNavigationFlow?.staticReleaseNavigation?.tabName;
    if (!activeTabName) return [] as StaticReleaseNavigation[];
    return [...(staticSteps[activeTabName] || [])].sort(
      (a, b) => a.stepOrder - b.stepOrder,
    );
  }, [activeReleaseNavigationFlow?.staticReleaseNavigation?.tabName, staticSteps]);

  const activeStepIndex = navigationSteps.findIndex(
    (step) => step.id === activeReleaseNavigationFlow?.staticReleaseNavigationId,
  );
  ...
  return (
    <article className="overflow-hidden rounded-md bg-white shadow-sm w-full">
      <section
        className={`w-full ${hasStepSidebar ? 'grid gap-0 lg:grid-cols-[300px_minmax(0,1fr)]' : 'block'
          }`}
      >
        {hasStepSidebar && (
          <aside className="bg-gray-50/80 lg:border-r">
            ...
            <nav className="flex flex-col gap-2 p-3" aria-label="Release wizard steps">
              {navigationSteps.map((step, index) => {
                ...
                return (
                  <Link
                    ...
                    className={`relative flex items-center gap-3 rounded-md border px-3 py-3 text-left transition-all duration-200 ${isActive
                      ? 'border-primary/25 bg-primary/8 shadow-[0_6px_14px_-12px_rgba(0,90,150,0.35)]'
                      : 'border-transparent bg-white hover:border-gray-200 hover:bg-gray-50'
                      }`}
                  >
                    <span
                      className={`flex size-8 shrink-0 items-center justify-center rounded-full text-xs font-normal ${isCompleted
                    ...
                    <span className="min-w-0 flex-1">
                      <span
                        className={`block truncate text-[12px] font-normal ${isActive ? 'text-primary' : 'text-gray-700'
                          }`}
                      >
                ...
                <ul className='flex items-center gap-2 mt-1'>
                  <p className="text-[12px] text-gray-500">
                    {isLoading ? (
                      ...
                      release?.title || 'Fill out each section to prepare this release.'
                    )}
                  </p> <span className='text-[12px] text-gray-500'>•</span> {release?.catalogNumber && <p className='text-[12px] text-gray-500'>{release?.catalogNumber} <FontAwesomeIcon className='text-gray-500 text-[11px] ml-0.5 cursor-pointer' icon={copied ? faCircleCheck : faCopy} onClick={(e) => { e.preventDefault(); navigator.clipboard.writeText(release?.catalogNumber || ''); setCopied(true); setTimeout(() => setCopied(false), 2000); toast.success('Copied to clipboard'); }} /></p>}
                </ul>
              </menu>
              {activeReleaseNavigationFlow && !isLoading && (
                <span
                  className={`inline-flex w-fit items-center rounded-full px-3 py-1 text-xs font-normal ${activeReleaseNavigationFlow?.completed
                    ? 'bg-primary/10 text-primary'
                    : 'bg-gray-100 text-gray-500'
                    }`}
                >
                  {activeReleaseNavigationFlow.completed ? 'Completed' : 'In progress'}
                </span>
              )}
            </nav>
          </header>
