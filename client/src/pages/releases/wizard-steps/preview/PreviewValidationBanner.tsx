...
const PreviewValidationBanner = ({
  ...
  return (
    <aside className="rounded-md border border-[color:var(--lens-ink)]/20 bg-[color:var(--lens-sand)]/30 p-4 shadow-xs">
      <header className="flex items-center gap-2">
        <FontAwesomeIcon
          icon={faExclamationTriangle}
          className="text-[12px] text-[color:var(--lens-ink)]"
        />
        <h3 className="text-[12px] font-medium text-[color:var(--lens-ink)]">
          Validation failed
      ...
      <ul className="mt-2 flex flex-col gap-1 pl-5">
        {validationResult?.errors?.map((error) => (
          <li key={error} className="list-disc text-[11px] text-[color:var(--lens-ink)]/75">
            {error}
          </li>
        ))}
      </ul>
